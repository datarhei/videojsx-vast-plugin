...
import { once, convertOffsetToSeconds } from './utils.mjs';
...
export class VastPlugin extends Plugin {
    ...
    constructor(player, options) {
        ...
        const onTimeUpdate = (() => {
            ...
            return () => {
                ...
                if (offsetInSeconds != null) {
                    ...
                    let currentTime = playerTime - startOffset;
                    if (options.debug === true) {
                        console.log('playerTime', playerTime.toFixed(6), 'startOffset', startOffset.toFixed(6), 'currentTime', currentTime.toFixed(6), 'nextBreak in', (offsetInSeconds - currentTime).toFixed(6));
                    }
                    if (currentTime > offsetInSeconds) {
                        ...
                        if (typeof scheduleItem.rewrite === 'function') {
                            scheduleItem.url = scheduleItem.rewrite(scheduleItem.url);
                        }
                        adLoader
                            .loadAds(scheduleItem)
                            .then((trackedAds) => {
        ...
        const createSourceObjects = (mediaFiles) => {
            return mediaFiles.filter((mediaFile) => mediaFile.apiFramework == null).map((mediaFile) => ({ type: mediaFile.mimeType, src: mediaFile.fileURL }));
        };
